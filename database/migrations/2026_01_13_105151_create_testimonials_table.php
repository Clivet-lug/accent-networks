<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('testimonials', function (Blueprint $table) {
            $table->id();
            $table->string('client_name'); // Person giving testimonial
            $table->string('company')->nullable(); // Their company
            $table->string('position')->nullable(); // Their position (e.g., "CEO")
            $table->text('content'); // The testimonial text
            $table->string('avatar')->nullable(); // Profile photo
            $table->integer('rating')->default(5); // 1-5 stars
            $table->boolean('is_featured')->default(false); // Show on homepage
            $table->boolean('is_published')->default(true); // Published or hidden
            $table->integer('order')->default(0); // Manual sorting
            $table->timestamps();
            $table->softDeletes();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('testimonials');
    }
};
